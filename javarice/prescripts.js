...
const PRESCRIPTS_STORAGE_KEY = "PrescriptMessages";

function normalizePrescript(input) {
  const source = input && typeof input === "object" ? input : {};
  const weight = Number(source.weight);
  const minCount = Number(source.minCount);
  const amounts = Number(source.amounts);

  return {
    text: typeof source.text === "string" ? source.text.trim() : "",
    weight: Number.isFinite(weight) ? Math.max(0, Math.round(weight * 10) / 10) : 0,
    minCount: Number.isFinite(minCount) ? Math.max(0, Math.floor(minCount)) : 0,
    amounts: Number.isFinite(amounts) ? Math.max(0, Math.floor(amounts)) : 0,
  };
}

function clonePrescripts(source) {
  return source.map(item => ({ ...item }));
}

function isValidPrescript(item) {
  return item.text.length > 0 && item.weight > 0 && item.amounts >= 0;
}

function loadPrescripts() {
  try {
    const stored = localStorage.getItem(PRESCRIPTS_STORAGE_KEY);
    if (stored === null) {
      return clonePrescripts(defaultPrescripts);
    }
    ...
    const normalized = parsed.map(normalizePrescript).filter(isValidPrescript);
    return normalized;
  } catch (error) {
    console.warn("Unable to load saved prescripts. Falling back to defaults.", error);
    return clonePrescripts(defaultPrescripts);
  }
}

let messages = loadPrescripts();
const recentMessagesSize = 5;
const recentMessages = [];

function savePrescripts() {
  try {
    localStorage.setItem(PRESCRIPTS_STORAGE_KEY, JSON.stringify(messages));
    return true;
  } catch (error) {
    console.warn("Unable to save prescripts.", error);
    return false;
  }
}

function resetSelectionMemory() {
  recentMessages.length = 0;
  lastMessage = null;
}

function getPrescripts() {
  return clonePrescripts(messages);
}

function replacePrescripts(nextPrescripts) {
  const next = Array.isArray(nextPrescripts)
    ? nextPrescripts.map(normalizePrescript).filter(isValidPrescript)
    : [];

  messages = next;
  resetSelectionMemory();
  savePrescripts();
  return getPrescripts();
}

function getDefaultPrescripts() {
  return clonePrescripts(defaultPrescripts);
}

function resetPrescripts() {
  messages = getDefaultPrescripts();
  resetSelectionMemory();
  savePrescripts();
  return getPrescripts();
}

window.prescriptStore = {
  get: getPrescripts,
  getDefaults: getDefaultPrescripts,
  replace: replacePrescripts,
  reset: resetPrescripts,
  storageKey: PRESCRIPTS_STORAGE_KEY,
};
...
function pickMessage() {
  ...
  while (attempts < MAX_ATTEMPTS) {
    ...
    if (picked && picked !== lastMessage && !recentMessages.includes(picked)) {
      break;
    }
